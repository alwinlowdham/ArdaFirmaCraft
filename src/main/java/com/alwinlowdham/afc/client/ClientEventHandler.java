...
public class ClientEventHandler {
    ...
    public static void checkAttack(boolean thrust, InputEvent.ClickInputEvent event) {
        ...
        if (thrust && !(mc.player.getMainHandItem().getItem() instanceof AFCWeaponItem)) {
            return;
        }
        ...
        if (missTime <= 0) {
            if (mc.hitResult == null) {
                ...
                switch (mc.hitResult.getType()) {
                    case ENTITY:
                        AFCNetwork.sendPacketToServer(new AttackPacket(thrust, ((EntityRayTraceResult) mc.hitResult).getEntity().getId()));
                        break;
